...
PROFILEPROD_SURFACE = PROFILEPRODSURFACE;
...
def _3dBoxFromBezierBorders(borders):
    resultBox = None;
    for item in borders:
        bezierCurve = BEZIER(S1)(item);
        surface3d = MAP(bezierCurve)(PROD([INTERVALS(1)(dom),INTERVALS(1)(dom)]));
...
def translatePointsEqually(pts):
    def t1(dx, dy, dz):
        result = [];
        for coord in pts:
            newcoord = [];
            newcoord.append(coord[0] + dx);
            newcoord.append(coord[1] + dy);
            newcoord.append(coord[2] + dz);
            result.append(newcoord);
        return result;
    return t1;

#translate points for a specific type of bezier curve
def translatePoints(pts):
    def function(dx, dy, dz):
        result = [];
        for i in range(4):
            newcoord = [];
            coord = pts[i];
...
cylPillowBarDisk = T([x,y])([3-0.084,4-0.084])(CIRCLE(0.02)([dom,1]));
cylPillowBarDiskTop = T([x,y,z])([3-0.084,4-0.084, 1.45])(CIRCLE(0.02)([dom,1]));
...
dom2D = PROD([INTERVALS(1)(dom),INTERVALS(1)(dom)]);
